...
var subdomainVisits = function (cpdomains) {
  ...
  cpdomains.forEach((domain) => {
    let count = Number(domain.split(' ')[0]);

    if (!domainCollection[domain]) {
      let domainName = domain.split(' ')[1];
      domainCollection[domainName] = count;

      let subdomains = domain.split('.').splice(1);

      subdomains.forEach((subdomain) => {
        domainCollection[subdomain] ? domainCollection[subdomain] += count : domainCollection[subdomain] = count;
      })
    }
  });

  let subdomainVisitsArr = Object.values(domainCollection);
  return subdomainVisitsArr.map((count, index) => {
    return count += ` ${Object.keys(domainCollection)[index]}`;
  })
};
